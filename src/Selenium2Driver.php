...
class Selenium2Driver extends CoreDriver
{
    ...
    public function setRequestHeader($name, $value)
    {
        ...
        if ($lowerName === 'user-agent') {
            if ($this->webDriver) {
                throw new DriverException('Unable to update desiredCapabilities, the session has already started');
            }

            $desiredCapabilities = $this->getDesiredCapabilities();

            if ($desiredCapabilities->getBrowserName() === 'chrome') {
                $options = $desiredCapabilities->getCapability(ChromeOptions::CAPABILITY);
                $options['args'] = array_merge($options['args'], array('user-agent='.$value));
                $desiredCapabilities->setCapability(ChromeOptions::CAPABILITY, $options);
            } else if ($desiredCapabilities->getBrowserName() === 'firefox') {
                // general.useragent.override
                $options = $desiredCapabilities->getCapability(FirefoxDriver::PROFILE);
                $options->setPreference('general.useragent.override', $value);
            }

            return;
        }

        parent::setRequestHeader($name, $value); // TODO: Change the autogenerated stub
    }
